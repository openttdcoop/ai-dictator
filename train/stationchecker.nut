	...
	do	{
		...
		if (success)	{
					...
					if (tmptaker)
						...
						if (cTileTools.CanUseTile(fromtile,stationObj.s_ID)) // it's crossing+rightTileOf
...
function cBuilder::RailStationPhaseSignalBuilder(road)
	...
	if (road.Source_RailEntry)
			srcpos=road.SourceStation.s_EntrySide[TrainSide.IN];
		else	srcpos=road.SourceStation.s_ExitSide[TrainSide.IN];
	if (road.Target_RailEntry)
			dstpos=road.TargetStation.s_EntrySide[TrainSide.OUT];
	...
	cDebug.ClearSigns();
	if (road.Source_RailEntry)
			srcpos=road.SourceStation.s_EntrySide[TrainSide.OUT];
		else	srcpos=road.SourceStation.s_ExitSide[TrainSide.OUT];
	if (road.Target_RailEntry)
			dstpos=road.TargetStation.s_EntrySide[TrainSide.IN];
...
//	if (!cStationRail.IsRailStationExitOpen(staID) && !useEntry)	{ DWarn(thatstation.name+" exit is CLOSE",1); return false }
	...
	INSTANCE.main.builder.SetRailType(thatstation.s_SubType); // not to forget
	local success=false;
